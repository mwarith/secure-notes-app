...
| §10–§11 Logging & Metrics | Infra provisioned (ENG-1); JSON logger + counter seams (ENG-15, registry process-wide per ENG-39); `/api/metrics` Prometheus exposition (ENG-39), restart-aware per ENG-54 (non-sparse counters, `app_process_start_time_seconds` gauge, `_created` lines — fixes drill finding F2's silent-reset drop); provisioned Grafana dashboards over the real counters + Loki panels (ENG-40) — pino swap pending (ENG-16) | ENG-1, ENG-15, ENG-33–35, ENG-39, ENG-40, ENG-54 | Merged; drill re-run: `/api/metrics` and Prometheus agree across outage + recovery (`docs/cache-drill.md` drill log). ENG-16 pending |
